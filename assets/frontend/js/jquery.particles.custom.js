(function($) {
  ...
  function particles_background_script($scope) {
    ...
    particles_style = $scope.find('.df-particle-wrapper').data('df-partstyle');

    if (typeof particles_style != 'undefined') {
      if (element_type == 'column') {
        $column = $scope.find('.elementor-column-wrap .elementor-background-overlay');
        $particle_wrap = $scope.find('.df-particle-wrapper');
        if ($column.next('#df-particle-' + id).length == 0) {
          $column.after($particle_wrap);
        }
      } else {
        $bg_wrap = $scope.find('.elementor-element-overlay ~ .elementor-background-overlay');
        $particle_wrap = $scope.find('#df-particle-' + id);
        if ($bg_wrap.next('#df-particle-' + id).length == 0) {
          $bg_wrap.after($particle_wrap);
        }
      }
      $selector = $scope.find('#df-particle-' + id);

    } else {

      particles_style = $scope.data('df-partstyle');

      $selector = $scope;
      $content = $('<div class="df-particle-wrapper" id="df-particle-' + id + '"></div>');
    ...
    var particles_dot_color = $selector.data('df-partcolor'),
      particles_custom = $selector.data('df-partdata'),
      particles_opacity = $selector.data('df-partopacity'),
      particles_direction = $selector.data('df-partdirection');

    if (particles_custom !== '' && particles_style == 'custom') {
      particlesJS('df-particle-' + id, particles_custom);
      ...
      if ($scope.hasClass('df-particle-adv-yes')) {
        var particles_number = $selector.data('df-partnum');
        var particles_size = $selector.data('df-partsize');
        var particles_speed = $selector.data('df-partspeed');
        var particles_interactive = $selector.data('df-interactive');
      ...
      particlesJS('df-particle-' + id, config);
    }
  }
